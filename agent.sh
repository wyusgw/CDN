...
install_depend() {
    if check_sys sysRelease ubuntu;then
        apt-get update
        apt-get -y install wget python3-minimal
...
if [[ $MASTER_VER == "" ]]; then
    ...
    agent_ver=`(curl -s -m 5 "http://auth.fikkey.com/master/upgrades?version_num=$version_num" || curl -s -m 5 "https://github.com/SkyAerope/cdnfly/raw/main/web/upgrades?version_num=$version_num") | grep -Po '"agent_ver":"\d+"' | grep -Po "\d+" || true`
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本"
        exit 1
    fi
...
download "https://github.com/wyusgw/CDN/releases/download/v1.11.0/cdnfly-agent-v5.1.15.zip" "https://github.com/wyusgw/CDN/releases/download/v1.11.0/cdnfly-agent-v5.1.15.zip" "$tar_gz_name"
